   ...
   Target CreateBundle => _ => _
       ...
       .Executes(() =>
       {
          foreach (var project in Bundles)
          {
             ...
             var pathPackage = Path.Combine(project.Directory, "PackageContents.xml");

             Log.Information("pathPackage", pathPackage);

             File.Copy(pathPackage, Path.Combine(bundlePath, Path.GetFileName(pathPackage)), true);
